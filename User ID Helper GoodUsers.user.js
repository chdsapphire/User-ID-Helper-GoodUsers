...
(function() {
    ...
    const createButton = (elem, userId) => {
        const button = $('<button id="test-button">❓</button>');
        button.css({
            'position': 'absolute',
            'margin-left': '-20px',
            'font-size': '10px',
        ...
        button.hover(function () {
            const link = `http://www.charmdate.com/clagt/livechat/search_result.php?postdate_s_y=2001&postdate_s_m=01&postdate_s_d=01&postdate_e_y=2023&postdate_e_m=12&postdate_e_d=31&status=close&service_type=&manid=${userId}`;
            window.open(link, "_blank");
        });
        elem.prepend(button);
    };

    // Find all elements that contain text that starts with "CM"
    const elements = Array.from(document.getElementsByTagName("*")).filter((elem) => {
        return elem.textContent.trim().startsWith("CM");
    });

    // Add a small button before each matching element
    elements.forEach((elem) => {
        const text = elem.textContent.trim();
        const userId = text.match(/CM\d+/)[0];
        createButton(elem, userId);
    });
})();
